...
@function
def DownPlunge(bedseg=None,trd=None,plg=None,*args,**kwargs):
    varargin = DownPlunge.varargin
    nargin = DownPlunge.nargin
...
#equation 3.10
    a[1,1]=dot(sin(trd),sin(plg))
# DownPlunge.m:32
    a[1,2]=dot(cos(trd),sin(plg))
# DownPlunge.m:33
    a[1,3]=cos(plg)
# DownPlunge.m:34
    a[2,1]=cos(trd)
...
# DownPlunge.m:36
    a[2,3]=0.0
# DownPlunge.m:37
    a[3,1]=dot(sin(trd),cos(plg))
# DownPlunge.m:38
    a[3,2]=dot(cos(trd),cos(plg))
# DownPlunge.m:39
    a[3,3]=- sin(plg)
# DownPlunge.m:40
...
#bed. The details of this algorithm are described in Chapter 4
    for nv in arange(1,nvtex).reshape(-1):
        for i in arange(1,3).reshape(-1):
            dpbedseg[nv,i]=0.0
# DownPlunge.m:49
            for j in arange(1,3).reshape(-1):
                dpbedseg[nv,i]=dot(a[i,j],bedseg[nv,j]) + dpbedseg[nv,i]
# DownPlunge.m:51
