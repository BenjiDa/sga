...
@function
def PrincipalStress(stress=None,tX1=None,pX1=None,tX3=None,*args,**kwargs):
    varargin = PrincipalStress.varargin
    nargin = PrincipalStress.nargin
...
# PrincipalStress.m:27
    #Initialize pstress
    pstress=zeros(3,3)
# PrincipalStress.m:30
...
#are the corresponding eigenvectors (i.e. principal stress directions)
    V,D=eig(stress,nargout=2)
# PrincipalStress.m:36
    #Fill principal stress magnitudes
    pstress[1,1]=D[3,3]
# PrincipalStress.m:39
    
    pstress[2,1]=D[2,2]
# PrincipalStress.m:40
    
    pstress[3,1]=D[1,1]
# PrincipalStress.m:41
...
#transformed to the North-East-Down coordinate system (e.g. Eq. 3.9)
    tV=zeros(3,3)
# PrincipalStress.m:46
    for i in arange(1,3).reshape(-1):
        for j in arange(1,3).reshape(-1):
            for k in arange(1,3).reshape(-1):
                tV[j,i]=dot(dC[k,j],V[k,i]) + tV[j,i]
# PrincipalStress.m:50
    
    #Initialize dCp
    dCp=zeros(3,3)
# PrincipalStress.m:56
    #Trend and plunge of maximum principal stress direction
    dCp[1,:]=cat(tV[1,3],tV[2,3],tV[3,3])
# PrincipalStress.m:59
    pstress[1,2],pstress[1,3]=CartToSph(tV[1,3],tV[2,3],tV[3,3],nargout=2)
# PrincipalStress.m:60
    #Trend and plunge of intermediate principal stress direction
    dCp[2,:]=cat(tV[1,2],tV[2,2],tV[3,2])
# PrincipalStress.m:63
    pstress[2,2],pstress[2,3]=CartToSph(tV[1,2],tV[2,2],tV[3,2],nargout=2)
# PrincipalStress.m:64
    #Trend and plunge of minimum principal stress direction
    dCp[3,:]=cat(tV[1,1],tV[2,1],tV[3,1])
# PrincipalStress.m:67
    pstress[3,2],pstress[3,3]=CartToSph(tV[1,1],tV[2,1],tV[3,1],nargout=2)
# PrincipalStress.m:68
    return pstress,dCp
