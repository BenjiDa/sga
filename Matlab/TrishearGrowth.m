function frames = TrishearGrowth(yp,psect,tparam,sinc,G)
...
for i=1:ninc
   ...
   %Get frame for movie
   frames(i) = getframe;
   %Add growth strata. Careful: Intersections pregrowth-growth strata are
   %not calculated. Growth strata will not look right for subsidence rate
   %lower than uplift rate G < 1.0
   if (i == countG*nincG)
       %Make growth strata
       %Update top
       top = top + nincG*sinc*sin(ramp)*G;
       % Make bed geometry
       xm=i*sinc:extent/npoint:extent+i*sinc;
       [GXP,GYP]=meshgrid(xm,top);
       %Transform to coordinate axes parallel and perpendicular to the 
